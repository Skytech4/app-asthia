...
def file_download(df):
    ...
    href = f'<a href="data:file/csv;base64,{b64}" download="dataset_asthma.csv">Download CSV File</a>'
    return href
...
with open("nettoyage/ml_model_asthma.pkl", "rb") as file:
...
TRANSLATIONS = {
    ...
    "Sélectionnez un enregistrement audio pour l'analyse sonore": {
        "fr": "Sélectionnez un enregistrement audio pour l'analyse sonore",
        "en": "Select an audio recording for the sound analysis"
    },
    ...
    "Enregistrement sélectionnée": {
        "fr": "Enregistrement sélectionnée",
        "en": "Recording selected"
    },
    ...
    "Quels sont les objectifs d'Asthia ?": {
        "fr": "Quels sont les objectifs d'Asthia ?",
        "en": "What are the objectives of Asthia?"
    },
    "Asthia est une application basée sur l'intelligence artificielle qui a pour objectif d'améliorer la prise en charge des patients asthmatiques. Elle vise à fournir des outils de diagnostic et de suivi personnalisés, facilitant ainsi la gestion de cette maladie chronique. Spécifiquement dit :": {
        "fr": "Asthia est une application basée sur l'intelligence artificielle qui a pour objectif d'améliorer la prise en charge des patients asthmatiques. Elle vise à fournir des outils de diagnostic et de suivi personnalisés, facilitant ainsi la gestion de cette maladie chronique. Spécifiquement dit :",
        "en": "Asthia is an artificial intelligence-based application aimed at improving the management of asthma patients. It aims to provide personalized diagnostic and monitoring tools, thereby facilitating the management of this chronic disease. Specifically stated:"
    },
    "- La détection de l'asthme grâce à un formulaire de constantes cliniques en entrée par l'utilisateur ;": {
        "fr": "- La détection de l'asthme grâce à un formulaire de constantes cliniques en entrée par l'utilisateur ;",
        "en": "- The detection of asthma through a form of clinical constants entered by the user ;"
    },
    "- La détection des anomalies liées à la fonction respiratoire grâce à l'analyse des données spécifiques telles que de l'audio respiratoire ;": {
        "fr": "- La détection des anomalies liées à la fonction respiratoire grâce à l'analyse des données spécifiques telles que de l'audio respiratoire ;",
        "en": "- The detection of anomalies related to respiratory function through the analysis of specific data such as respiratory audio ;"
    },
    "- L'interaction rapide avec un chatbot intelligent pour l'obtention de toute interrogation concernant la condition et les résultats d'analyse.": {
        "fr": "- L'interaction rapide avec un chatbot intelligent pour l'obtention de toute interrogation concernant la condition et les résultats d'analyse.",
        "en": "- Quick interaction with an intelligent chatbot to obtain any inquiries regarding the condition and analysis results."
    }
}
...
def main():
    ...
    # Load dataset
    data = load_data("Dataset/dataset_asthma.csv")
    ...
    if choice == "Home":
        ...
        st.subheader(t("Quels sont les objectifs d'Asthia ?"))
        st.markdown("")
        st.markdown(f"""
        <div style="text-align: justify; font-size: 16px;">
        {t("Asthia est une application basée sur l'intelligence artificielle qui a pour objectif d'améliorer la prise en charge des patients asthmatiques. Elle vise à fournir des outils de diagnostic et de suivi personnalisés, facilitant ainsi la gestion de cette maladie chronique. Spécifiquement dit :")}
        </div>
        """, unsafe_allow_html=True)
        st.markdown("")
        st.markdown(f"""
        <div style="text-align: justify; font-size: 16px;">
        {t("- La détection de l'asthme grâce à un formulaire de constantes cliniques en entrée par l'utilisateur ;")}
        </div>
        """, unsafe_allow_html=True)
        st.markdown("")
        st.markdown(f"""
        <div style="text-align: justify; font-size: 16px;">
        {t("- La détection des anomalies liées à la fonction respiratoire grâce à l'analyse des données spécifiques telles que de l'audio respiratoire ;")}
        </div>
        """, unsafe_allow_html=True)
        st.markdown("")
        st.markdown(f"""
        <div style="text-align: justify; font-size: 16px;">
        {t("- L'interaction rapide avec un chatbot intelligent pour l'obtention de toute interrogation concernant la condition et les résultats d'analyse.")}
        </div>
        """, unsafe_allow_html=True)
        st.markdown("")
        ...
        count_gender= int(data[(data['Gender'] == 'Homme')]['Gender'].count())
        ...
        with col2:
            ...
            sns.boxplot(x="Gender", y="Age", data=data, palette='viridis')
            st.pyplot(fig4)
        ...
        with tab1:
            ...
            with tab11:
                ...
                with sstab1:
                        R5Hz_PP = st.number_input("R5Hz_PP", min_value=0.0, max_value=10.0, value=0.0, step=0.01)
                        R20Hz_PP = st.number_input("R20Hz_PP", min_value=0.0, max_value=10.0, value=0.0, step=0.01)
                ...
                if button1:
                    ...
                    if New_Asthma:
                        ...
                        class_index = 1 if New_Asthma == t("Asthmatique") else 0
            ...
            with tab12:
                ...
                if file_uploaded:
                    ...
                    colonnes_modele = ["Hydroxyurea", "ICS", "LABA", "Gender", "Age", "Height", "Weight", "BMI", "R5Hz_PP", "R20Hz_PP", "X5Hz_PP", "Fres_PP"]
                    ...
                    datas_csv["Gender"] = datas_csv["Gender"].map({"Male": 1, "Female": 0})

                    ndf = None

                    if st.button(t("Prédiction"), key="predict_ml2"):
                        loaded_model1 = pickle.load(open("nettoyage/ml_model_asthma.pkl", "rb"))
                        ...
                        st.download_button(
                            label=t("Télécharger le fichier CSV"),
                            data=csv,
                            file_name='predictions.csv',
                            mime='text/csv',
                            key="download_csv_ml2"
                        )
        ...
        with tab2:
            ...
            deep_model = load_model('nettoyage/dl_model_asthma.h5')

            uploaded_file = st.file_uploader(t("Sélectionnez un enregistrement audio pour l'analyse sonore"), type=["wav"])

            if uploaded_file is not None:
                # Permettre à l’utilisateur d’écouter le fichier audio
                st.audio(uploaded_file, format='audio/wav')

                # Charger l'audio
                y, sr = librosa.load(uploaded_file, sr=None)

                # Conversion en melspectrogramme (128 x 128)
                ...
                # Normalisation et mise en forme pour le modèle
                mel_norm = (mel_resized - mel_resized.min()) / (mel_resized.max() - mel_resized.min())
                mel_input = np.expand_dims(mel_norm, axis=-1)   # (128,128,1)
                mel_input = np.expand_dims(mel_input, axis=0)   # (1,128,128,1)

                # Prédiction
                prediction = deep_model.predict(mel_input)
                prob = float(prediction[0][0])

                result_dl = t("Asthmatique") if prob > 0.5 else t("Normal")
                proba_text = round(prob * 100, 2) if result_dl == t("Asthmatique") else round((1 - prob) * 100, 2)
                ...
                fig = go.Figure(go.Indicator(
                    ...
                    gauge = {
                        'axis': {'range': [0, 100], 'tickwidth': 1, 'tickcolor': "white", 'tickfont': {'color': 'white'}},
                        'bar': {'color': "#d9534f" if result_dl == t("Asthmatique") else "#5cb85c"},
                        'bgcolor': "#000000",
                        'borderwidth': 2,
                        'bordercolor': "gray",
                        'steps': [
                        ...
                        'threshold': {
                            'line': {'color': "#d9534f" if result_dl == t("Asthmatique") else "#5cb85c", 'width': 4},
                            'thickness': 0.75,
                            'value': proba_text
                        }
                    }
